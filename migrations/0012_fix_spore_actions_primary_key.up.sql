-- Fix spore_actions primary key to support multiple actions per transaction
-- Add action_index column and create composite primary key

-- First, drop the existing primary key constraint
ALTER TABLE spore_actions DROP CONSTRAINT spore_actions_pkey;

-- Add action_index column to distinguish multiple actions within the same transaction
ALTER TABLE spore_actions ADD COLUMN action_index INTEGER NOT NULL DEFAULT 0;

-- Create composite primary key using both tx_hash and action_index
